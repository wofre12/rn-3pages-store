...
export default function AllProductsScreen(){
  ...
  React.useEffect(()=>{
    const sub = NetInfo.addEventListener((s: NetInfoState)=>setOnline(Boolean(s.isConnected)));
    return ()=>sub();
  },[]);
  ...
  return (
    <View style={{ flex:1, backgroundColor:bg, padding:12 }}>
      {!online && <Text style={{ textAlign:"center", padding:8, backgroundColor:"#ffeaea", color: text }}>Offline</Text>}
      <FlatList
        data={data ?? []}
